	...
	for "_check" from 1 to (1 + random 2) do {
		...
		for "_i" from 1 to (1 + random 1) do {
			...
			if (lambs_danger_debug_functions) then {
				_m = [_target,format ["%1 (Check round %2)",getText (configFile >> "CfgVehicles" >> (typeOf _gun) >> "displayName"),_i],"colorIndependent","hd_destroy"] call lambs_danger_fnc_dotMarker;
				_mlist pushBack _m;
			}; 
