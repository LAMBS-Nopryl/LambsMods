...
while {{alive _x} count units _grp > 0} do {
  ...
  // wait
  sleep _cycle; 
  if (lambs_danger_debug_functions) then {systemchat format ["danger.wp taskCQB: (team: %1) (units: %2) (enemies: %3)",groupID _grp,count units _grp,!isNull _e];};

};

// reset
if (lambs_danger_debug_functions) then {systemchat "danger.wp taskCQB: CQB DONE version 0.27";};
